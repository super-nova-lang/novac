...
/// Generate LLVM IR for the provided AST, returning it as a string.
/// This is the sole public entrypoint for the LLVM backend.
pub fn gen_target(module_name: &str, ast: &[Node]) -> Result<String> {
    let context = Context::create();
    let codegen = __private::Codegen::new(&context, module_name);
                        ...
                        Statement::Decl(decl) => match decl {
                            ...
                            DeclStmt::Decl {
                                ...
                                if let Some(expr) = &body.1 {
                                    match expr.as_ref() {
                                        Expression::StructExpr(_, Some(with_block))
                                        | Expression::EnumExpr(_, Some(with_block)) => {
                                            // collect parent params to prepend to inner decls
                                            let parent_params_clone = parent_params.clone();
                                            for node in with_block {
                                                if let Node::Statement(Statement::Decl(inner)) =
                                                    node
                                                {
                                                    ...
                                                    } = inner
                                                    {
                                                        ...
                                                        let mut combined_params =
                                                            parent_params_clone.clone();
                                                        combined_params
                                                            .extend(inner_params.clone());
        ...
        fn emit_relational(
            ...
            match rel {
                ...
                RelationalExpr::Eql(l, r) => {
                    self.emit_int_cmp(l, r, env, inkwell::IntPredicate::EQ)
                }
                RelationalExpr::Neq(l, r) => {
                    self.emit_int_cmp(l, r, env, inkwell::IntPredicate::NE)
                }
                RelationalExpr::Lt(l, r) => {
                    self.emit_int_cmp(l, r, env, inkwell::IntPredicate::SLT)
                }
                RelationalExpr::Gt(l, r) => {
                    self.emit_int_cmp(l, r, env, inkwell::IntPredicate::SGT)
                }
                RelationalExpr::Leq(l, r) => {
                    self.emit_int_cmp(l, r, env, inkwell::IntPredicate::SLE)
                }
                RelationalExpr::Geq(l, r) => {
                    self.emit_int_cmp(l, r, env, inkwell::IntPredicate::SGE)
                }
            }
        }
