...
let lookup_symbol ctx name =
  Hashtbl.find_opt ctx.symbols name
...
let get_symbol_type info =
  ...
  | Some t -> t
  | None -> match !(info.inferred_typ) with
    | Some t -> t
...
and infer_relational_type ctx = function
  | A.Eql (left, right) | A.Neq (left, right) | A.Lt (left, right)
  | A.Gt (left, right) | A.Leq (left, right) | A.Geq (left, right) ->
...
and analyze_match_arm ctx (param, match_if, body) =
  ...
  (match match_if with Some expr -> analyze_expression ctx' expr | None -> ());
...
and analyze_decl ctx = function
  | A.Decl { tags = _tags; name; params; explicit_ret; body = stmts, expr_opt } ->
    ...
    add_symbol ctx name explicit_ret (0, 0); (* TODO: get actual location *)
    let ctx' = enter_scope ctx in
...
let check_unused_variables ctx =
  Hashtbl.iter (fun name info ->
    Printf.printf "Checking symbol: %s, used: %b, scope: %d\n" name info.used info.scope_level;
