...
let function_protos : (string, L.lltype * A.typ) Hashtbl.t = Hashtbl.create 10 (* lltype is func type, A.typ is ret type *)
...
let variant_map : (string, string * string) Hashtbl.t = Hashtbl.create 10 (* variant -> (enum_name, func_name) *)
...
let find_struct_type name =
  try Some (Hashtbl.find named_struct_types name, name)
  with Not_found ->
    let found = ref None in
    Hashtbl.iter (fun k v -> if String.lowercase_ascii k = String.lowercase_ascii name then found := Some (v, k)) named_struct_types;
    !found
...
let get_variant_index enum_name variant_name =
  try
    let map = Hashtbl.find variant_indices enum_name in
...
let rec codegen_atom = function
  | A.Implicit_member m ->
      (try
       ...
       with Not_found -> raise (Error ("Unknown variant: " ^ m)))
  ...
  | A.Ident name ->
    (try 
       ...
       let t = Hashtbl.find named_value_types name in
       v, t
     with
     | Not_found ->
       (try 
          let v = Hashtbl.find global_values name in
          let t = try snd (Hashtbl.find function_protos name) with Not_found -> A.User "i32" in
          v, t
        with Not_found ->
          (match L.lookup_function name the_module with
           | Some f -> 
               let t = try snd (Hashtbl.find function_protos name) with Not_found -> A.Unit_typ in
               f, t
...
and codegen_call = function
  | A.Decl_call (callee, params) ->
    let func_name_opt, method_obj = 
      match callee with 
      | A.Relational_expr (A.Relational_val (A.Additive_val (A.Multiplicative_val (A.Unary_val (A.Ident name))))) -> Some name, None
      | A.Relational_expr (A.Relational_val (A.Additive_val (A.Multiplicative_val (A.Unary_member (obj_unary, member))))) -> 
          (match obj_unary with
           | A.Unary_val (A.Ident maybe_type) ->
               (match find_struct_type maybe_type with
                ...
                | None ->
                    let obj_expr = A.Relational_expr (A.Relational_val (A.Additive_val (A.Multiplicative_val obj_unary))) in
              ...
              (match obj_typ with
               | A.User type_name -> 
                   (match find_struct_type type_name with
                    ...
                    | None -> Some (type_name ^ "_" ^ member), Some obj_val)
               | _ -> None, None))
      | A.Relational_expr (A.Relational_val (A.Additive_val (A.Multiplicative_val (A.Unary_val (A.Implicit_member m))))) ->
          (try Some (snd (Hashtbl.find variant_map m)), None with Not_found -> None, None)
      | _ -> None, None
    in
        ...
        (match func_name_opt with
         | Some name ->
           (try
              ...
              in
              call_res, ret_ast_type         with Not_found -> 
           if Hashtbl.mem named_struct_types name then
             raise (Error ("Function prototype not found for: " ^ name))
           else
             (* Declare missing function as extern to allow compilation *)
             let ret_ast_type = if String.ends_with ~suffix:"_show" name then A.User "string" else A.User "i32" in
             let ret_type = get_llvm_type ret_ast_type in
             let actual_args = match method_obj with Some obj -> obj :: args | None -> args in
  ...
  | A.Macro_call (callee, params) ->
     let name =
       match callee with
       | A.Relational_expr (A.Relational_val (A.Additive_val (A.Multiplicative_val (A.Unary_val (A.Ident s))))) -> s
       | _ -> "macro"
     in
     if name = "println" || name = "my_printf" then (
       let printf =
       ...
       let fmt_arg, other_args = 
         match params with
         | A.Positional (A.Relational_expr (A.Relational_val (A.Additive_val (A.Multiplicative_val (A.Unary_val (A.String s)))))) :: rest -> 
       ...
       L.const_null i32_type, A.User "void"
     )
      ...
      (match obj_typ with
       | A.User type_name ->
          (try 
            ...
            res, field_ast_type
          with _ -> 
            raise (Error ("Unknown field " ^ member ^ " in struct " ^ type_name)))
         ...
         (fun i (param, _if_opt, body) ->
            ...
            let variant_name, var_bind =
              ...
              | A.Single
                  (Relational_expr
                     (Relational_val
                        (Additive_val (Multiplicative_val (Ast.Unary_val (A.Implicit_member m))))))
                -> m, None
              | A.Single
                  (Relational_expr
                     (Relational_val
                ...
                let v =
                  ...
                  | [ A.Positional
                        (Relational_expr
                           (Relational_val
                              (Additive_val (Multiplicative_val (Ast.Unary_val (A.Ident s)))))) ] -> Some s
                  | _ -> None
                in
                m, v
              | _ -> "unknown", None
            ...
            let cond =
              L.build_icmp L.Icmp.Eq tag_val (L.const_int i32_type expected_tag) "tag_eq" builder
            in
            ...
            (match var_bind with
             | Some s ->
               let data_ptr_ptr = L.build_struct_gep lenum target_val 1 "data_ptr_ptr" builder in
               let data_ptr = L.build_load (L.pointer_type context) data_ptr_ptr "data_ptr" builder in
               Hashtbl.add named_values s data_ptr;
               Hashtbl.add named_value_types s (A.User "string") (* Hack: assume string data *)
             | None -> ());
...
and codegen_decl = function
  | A.Decl
    ...
    L.struct_set_body lstruct (Array.of_list field_types) false;
                ...
                let the_function = L.declare_function name func_type the_module in
                Hashtbl.add function_protos name (func_type, A.User struct_name);
                ...
                let struct_alloca =
                  L.build_call
                    ...
                    (match L.lookup_function "malloc" the_module with
                     | Some f -> f
                     | None -> L.declare_function "malloc" (L.function_type (L.pointer_type context) [| i64_type |]) the_module)
                    [| struct_size_i64 |]
                    "struct"
                    builder
                in
  ...
  | A.Decl
    ...
    List.iteri
      (fun i (vname, vbody) ->
         ...
         let variant_func_name = name ^ "_" ^ vname in
  ...
  | A.Decl { name; params; body; explicit_ret; tags = _ } ->
    if !in_function
    then (
