...
let doc_files_with_stdlib stdlib_flag files =
  if stdlib_flag then List.map fst (get_stdlib_files ()) @ files else files
;;

let string_of_doc stdlib_flag out_type files =
  let doc_files = doc_files_with_stdlib stdlib_flag files in
  Doc_gen.string_of_doc ~doc_files ~out_type
;;
...
let collect_snapshot file =
  ...
  let doc_r = capture_output (fun () -> string_of_doc false Doc.Stdout [ file ]) in
...
let doc_cmd =
  ...
  let out_type_arg =
    let doc = "Output format: stdout or html (default: html)." in
    Arg.(
      value
      & opt (enum [ "stdout", Doc.Stdout; "html", Doc.Html ]) Doc.Html
