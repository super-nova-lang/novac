...
impl<'de> Codegen<'de> {
    ...
    fn emit_expr_to_rax_with_locals(
        ...
        match &annotated_expr.expr {
            ...
            Expr::Literal(Literal::String(s)) => {
                ...
                self.emitter.write_comment(&format!("string literal: {}", s));
    ...
    fn emit_builtin_call(
        ...
        match name {
            ...
            _ => {
                ...
                self.emitter.write_comment(&format!("unknown builtin: @{}", name));
                self.emitter.write_txt("mov rax, 0");
            }
        }
    }
    ...
    fn emit_print_builtin(
        ...
        if args.is_empty() {
            ...
            self.emitter.write_comment("print builtin called with no arguments");
            self.emitter.write_txt("mov rax, 0");
            return;
        }
        ...
        match inner_expr {
            Expr::Literal(Literal::String(s)) => {
                ...
                self.emitter.write_txt(&format!("mov rdi, {}", label));
            }
            _ => {
                // Dynamic string - evaluate to rax (should be a pointer)
                self.emit_expr_to_rax_with_locals(&format_annotated, params, locals);
                // Store in a temporary location and use it
                self.emitter.write_txt("mov [rbp - 8], rax"); // Use stack space
        ...
        if args.len() > 6 {
            self.emitter.write_comment("more than 6 arguments not fully supported");
        }
